...
from models import (Degree, Subject, Room, UserProfile, Lesson, CheckIn,
                ForumComment, Timetable, LessonComment, Building, AdminTask)
...
class TimeStateFilter(admin.SimpleListFilter):
    """
    Para filtrar un modelo segun su momento de inicio y fin, 
...
class SubjectStateFilter(TimeStateFilter):
    ...
    def queryset(self, request, queryset):
        ...
        if self.value() == 'now':
            return queryset.filter(last_date__gte = today,
...
class SolverFilter(admin.SimpleListFilter):
    """
    Para filtrar las AdminTask que ha resuelto el usuario que lo
    solicita
    """
    title = 'resuelto por'
