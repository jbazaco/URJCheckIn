	...
	def clean(self):
		...
		if self.first_date and self.last_date:
			if (self.first_date > self.last_date):
				raise ValidationError('La fecha de inicio no puede ser posterior a la de finalización')
...
class Lesson(models.Model):
	...
	codeword = models.CharField(max_length=20, verbose_name='código', default=get_rand_string)
	...
	def clean(self):
		...
		if self.start_time and self.end_time:
			if self.start_time < timezone.now():
				raise ValidationError('La hora de inicio debe ser posterior a este momento')
...
class CheckIn(models.Model):
	...
	mark = models.PositiveIntegerField(validators=[MaxValueValidator(5)], verbose_name='puntuación', blank=True)
	...
	def clean(self):
		...
		if not (self.codeword or self.longitude):
			raise ValidationError('Tienes que enviar el código o tu localización para hacer CheckIn')
		if self.codeword:
			if self.codeword != self.lesson.codeword:
				raise ValidationError('Ese código no se corresponde con el de la clase actual')
